...
function validateJWT(token, secret = process.env.SHOPIFY_API_SECRET) {
  ...
  const signatureCheck = crypto.createHmac("sha256", secret).update(`${header}.${payload}`).digest("base64");

  // Replace '+' with '-', '/' with '_', and remove '='
  const safeSignatureCheck = signatureCheck.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
