...
const Instruction = ({
  ...
  const markGuideAsDone = useCallback(async () => {
    try {
      if (!independentKey || independentKey.length === 0) return;
      const response = await fetch.put("/user/onboarding", { [`guide.${independentKey}`]: true });
      updateOnboardingData(response?.data);
      const guideData = response?.data?.guide;
  ...
  const patchData = useCallback(async () => {
    try {
      const guideData = onboardingData?.guide;
      setGuideData(guideData);
    } catch (err) {
      console.error(err);
    }
  }, [onboardingData, independentKey]);
  ...
  const InstructionContent = () => {
    return (
      <div
        className={`onboarding-instruction-container ${
          instruction.type === "video" && instruction?.video?.link?.length > 0 && "has-video"
        }`}
      >
        {instruction.type === "video" && instruction?.video?.link?.length > 0 && (
          <div
            className="instruction-video-container"
            style={{ width: !independent ? "360px" : "90px", height: !independent ? "220px" : "55px" }}
            onClick={() => {
              if (instruction?.video?.link?.length > 0) {
                setVideoModalLink(instruction.video.link);
                setVideoModal(true);
                setVideoModalTitle(instruction.title);
              }
            }}
          >
            <Image source={instruction.video.thumbnail}></Image>
          </div>
        )}
        <div className="instruction-content-container">
          <BlockStack gap="400" align="center">
            <InlineStack gap={100}>
              ...
              {independent && !instruction?.hideSkipButton && instruction?.status != "done" && (
                <Link
                  onClick={() => {
                    markGuideAsDone();
                  }}
            ...
            {!independent && <Text as="p">{instruction.description}</Text>}
            <InlineStack gap="200" align="start" blockAlign="center">
              {!independent &&
                instruction?.actions?.map(
                  (action, k) =>
                    action &&
                    action.content &&
                    action.onAction && (
                      <Button
                        key={k}
                        size={action.buttonSize || "medium"}
                        onClick={action.onAction}
                        variant={action.buttonVariant || "plain"}
              ...
              {!independent && !instruction?.hideSkipButton && instruction?.status != "done" && (
                <Button
                  ...
                  variant="tertiary"
                  onClick={() => {
                    if (typeof markAsDone === "function") {
                      markAsDone(stepKey, instructionKey);
                    }
                  }}
                >
                  Skip it
  ...
  return (
    <>
      {!guideData?.[independentKey] && (
        <div
          className={`instruction ${instruction.isOpen ? "open" : "closed"} status-${instruction.status}`}
          key={instructionKey}
        >
          {!independent && (
            <Box padding="0" borderRadius="200">
              <div className="instruction_title_container">
                <div
                  className="instruction_indicator"
                  onClick={() => {
                    markAsDone(stepKey, instructionKey);
                  }}
                >
                  <Tooltip
                    content={
                      instruction.status === "pending" ? t("common.Mark as done") : t("common.Mark as not done")
                    }
                    preferredPosition="above"
                  >
                    <span className={`instruction_indicator_icon ${instruction.status}`}>
                      {instruction.isLoading && <Button size="slim" loading plain></Button>}
                      {!instruction.isLoading && instruction.status === "pending" && (
                        <svg
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                      ...
                      {!instruction.isLoading && instruction.status === "done" && (
                        <svg
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          ...
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <circle cx="12" cy="12" r="10" fill="var(--p-color-bg-inverse)"></circle>
                          <path
                            d="M17.2738 8.52629C17.6643 8.91682 17.6643 9.54998 17.2738 9.94051L11.4405 15.7738C11.05 16.1644 10.4168 16.1644 10.0263 15.7738L7.3596 13.1072C6.96908 12.7166 6.96908 12.0835 7.3596 11.693C7.75013 11.3024 8.38329 11.3024 8.77382 11.693L10.7334 13.6525L15.8596 8.52629C16.2501 8.13577 16.8833 8.13577 17.2738 8.52629Z"
                            fill="var(--p-color-bg-surface-active)"
                          ></path>
                        </svg>
                      )}
                    </span>
                ...
                <div
                  className="instruction_title"
                  onClick={() => {
                    openCloseInstruction(stepKey, instructionKey);
                  }}
                >
                  <InlineStack blockAlign="center" align="space-between">
                    <Text
                      variant="headingMd"
                      as="span"
                      textDecorationLine={instruction.status === "done" ? "line-through" : ""}
                    >
                      {instruction.title}
                    </Text>
                    <InlineStack blockAlign="center" align="end" gap="200">
                      ...
                      <Button size="slim" variant="plain">
                        <Icon source={instruction.isOpen ? CaretUpIcon : CaretDownIcon} tone="base" />
                      </Button>
                    </InlineStack>
                  </InlineStack>
                </div>
              </div>
              <span className="status">{instruction.isOpen}</span>
              <Collapsible
                open={instruction.isOpen}
                id={"instruction-collapsible-" + instructionKey}
                expandOnPrint
              >
                <div className="instruction-collapsible">
          ...
          >
            <Modal.Section>
              <iframe
                src={videoModalLink}
                title={t("common.YouTube video player")}
